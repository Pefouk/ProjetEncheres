...
public class UtilisateursDAOJDBCIMPL implements UtilisateursDAO {
	...
	public Utilisateurs createUtilisateur(Utilisateurs u) throws DALException {
		...
		try (Connection con = ConnectionProvider.getConnection();
			...
			e.printStackTrace();
			throw new DALException("Erreur lors de la crï¿½ation de l'utilisateur : 1000");
		} finally {
			try {
				if (rs != null)
					rs.close();
	...
	public Utilisateurs connectUtilisateur(String email, String motDePasse) throws DALException {
		...
		try (Connection con = ConnectionProvider.getConnection();
			...
			if (rs.next()) {
				...
				throw new DALException("Conection ï¿½chouï¿½e, mot de passe ou email invalide");
		} catch (Exception e) {
	...
	private String getHashMDP(int id) throws DALException {
		...
		try (Connection con = ConnectionProvider.getConnection();
			...
			if (rs.next())
				...
				throw new DALException("Pas de hash de mot de passe trouvï¿½ : 2002");
		} catch (Exception e) {
			throw new DALException("Erreur lors de la rï¿½cuperation du hash de mot de passe : 2000");
		} finally {
			try {
				if (rs != null)
					rs.close();
